...
def image_completion_final(img,mask,Gs,latent):
        shift = 8
        mirrorSize = 4
        sizeVert = 100
        ...
        maskIn = PIL.Image.fromarray(mask).resize((shape,shape), PIL.Image.ANTIALIAS)
        maskIn = np.asarray(maskIn, dtype=np.float32)

        upper,upperMask = np.zeros(real.shape, dtype=np.uint8),np.zeros(maskIn.shape, dtype=np.uint8)
        lower,lowerMask = np.zeros(real.shape, dtype=np.uint8),np.zeros(maskIn.shape, dtype=np.uint8)
        
        upper[sizeVert:,:] = real[:shape-sizeVert,:]
        upperMask[sizeVert:,:] = maskIn[:shape-sizeVert,:]
        
       
        output = np.zeros(real.shape, dtype=np.uint8)
        if(1):
            saveReal = upper.copy()
            #Shift for improved result 
            start,end = findStartMask(maskIn[:shape-sizeVert,:])
            maskInShifted,realShifted = shiftL(shift,upperMask,upper,start,end)

            #Complete vertical shift with closest available pixels in the same column
            for w in range(realShifted.shape[1]):
                count = 0
                for h in range(realShifted.shape[0]):
                    if(maskInShifted[h,w]):
                        realShifted[count,w,:] = realShifted[h,w,:]
                        maskInShifted[count,w] = 1
                        count += 1
                    if(count==mirrorSize):
                        break
            ...
            output[:shape-sizeVert,:] = fakeShifted[sizeVert:,:]
            real[:shape-sizeVert,:] = fakeShifted[sizeVert:,:]
            maskIn[:shape-sizeVert,:] = 1
            
            lower[:shape-sizeVert,:] = real[sizeVert:,:]
            lowerMask[:shape-sizeVert,:] = maskIn[sizeVert:,:]

        if(1):
            saveReal = lower.copy()
            #Shift for improved result 
            start,end = findStartMask(maskIn[sizeVert:,:])
            maskInShifted,realShifted = shiftL(shift,lowerMask,lower,start,end)
            
            #Complete vertical shift with closest available pixels in the same column
            for w in range(realShifted.shape[1]):
                count = 1
                for h in range(realShifted.shape[0]-1,-1,-1):
                    if(maskInShifted[h][w]):
                        realShifted[-count,w,:] = realShifted[h,w,:]
                        maskInShifted[-count,w] = 1
                        count += 1
            ...
            fakeShifted = shiftR(shift,fake,saveReal,start,end)
            output[sizeVert:,:] = fakeShifted[:shape-sizeVert,:]

        output = cv2.resize(output, oriSize, interpolation = cv2.INTER_AREA)    
        return output
