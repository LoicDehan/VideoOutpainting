...
def extrapolation(args, video_ori, corrFlowF_ori, corrFlowB_ori,first):
    ...
    # Defines new FOV.
    imgH_extr = int(imgH)
    imgW_extr = int(imgW + args.W_scale)
...
def getFlowVideo(args,pathIn,pathOut):
    # Find frames.
    ...
    for filename in sorted(filename_list):
        ...
        if(ffirst):
            oriShape = img.shape
            args.W_scale = int(args.W_scale* 512/img.shape[0])
            ffirst=0
        if(img.shape[0]>512):
        ...
        for i in range(2560):
            ...
            if(lower<=W/scaleRaft<=upper):
                if(W-lower<upper-W):
                    ...
                    W_scale = upper
        imgRaft = cv2.resize(img, (H_scale,W_scale), interpolation = cv2.INTER_AREA) 
 
        print(H_scale,W_scale,imgRaft.shape)
    ...
    if(0):# args.mode == 'video_extrapolation':
        video2 = torch.stack(video2, dim=0)
        videoRaft2 = torch.stack(videoRaft2, dim=0)
        videoRaft2 = videoRaft2.to('cuda')
        # Flow model.
        RAFT_model = initialize_RAFT(args)
         # Calcutes the corrupted flow.
        corrFlowF2 = calculate_flow(pathOut, RAFT_model, videoRaft2, 'forward')
        corrFlowB2 = calculate_flow(pathOut, RAFT_model, videoRaft2, 'backward')
        corrFlowF2[:,:,:,:]*=scaleRaft
        corrFlowB2[:,:,:,:]*=scaleRaft
    
        # Convert to b,g,r,frame.

        video2 = video2.permute(2, 3, 1, 0).numpy()[:, :, ::-1, :] / 255.
        videoRaft2 = videoRaft2.permute(2, 3, 1, 0).cpu().numpy()[:, :, ::-1, :] / 255.
        corrFlowF_Resized2 = []
        corrFlowB_Resized2 = []
            
            
        for i in range(corrFlowF.shape[3]):
            corrFlowF_Resized2.append(cv2.resize(corrFlowF2[:,:,:,i], (H,W), interpolation = cv2.INTER_AREA))
            corrFlowB_Resized2.append(cv2.resize(corrFlowB2[:,:,:,i], (H,W), interpolation = cv2.INTER_AREA))
        
        corrFlowF2 = np.array(corrFlowF_Resized2)
        corrFlowB2 = np.array(corrFlowB_Resized2)
        
        corrFlowF2 =  np.transpose(corrFlowF2, axes=[1, 2, 3, 0])
        corrFlowB2 =  np.transpose(corrFlowB2, axes=[1, 2, 3, 0])
        return video,corrFlowF,corrFlowB,video2,corrFlowF2,corrFlowB2
...
def video_completion(Gs,args,video,mask,videoFlowF,videoFlowB,end_point,maskVOS,FG_removal):
    ...
    if(not(FG_removal)):# args.mode == 'video_extrapolation':
        ...
        t2 = time.perf_counter()   
        print("time spend flow completion inpaint:",t2-t1)
        global TimeInpFlowCompletion
        TimeInpFlowCompletion += t2-t1
...
def saveResult2(video_path,video):
    imgH, imgW,channels,nFrame = video.shape
...
def main(args):
    ...
    if(1):# args.mode == 'object_removal':
        out_path = os.path.join(args.outroot, 'object_removal')

        video,corrFlowF,corrFlowB = getFlowVideo(args,vid_path,out_path) 
        
        saveResult(os.path.join(args.outroot, 'input_video'),video)
        videoScaled = []
        W, H,_,_ = video.shape
        
        scaleVOS = (W*H)/200000 # Scale to Limit size of image on gpu to prevent out of memory
        for i in range(video.shape[3]):
            videoScaled.append(cv2.resize(video[:,:,:,i], (int(H/scaleVOS),int(W/scaleVOS)), interpolation = cv2.INTER_AREA))
    ...
    if(1):
        ...
        for filename in sorted(filename_list2):
            ...
            img = np.array(im)
            total_video[:,size_comp2:size_comp2+W0,:,i] = img
            total_video_blur[:,size_comp2:size_comp2+W0,:,i] = img
            i+=1
 
        video_path = os.path.join(args.outroot,'final_total')
        saveResult2(video_path,total_video)
        video_path = os.path.join(args.outroot,'final_blurred')
        saveResult2(video_path,total_video_blur)
